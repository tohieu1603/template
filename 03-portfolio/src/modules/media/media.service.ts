...
export class MediaService {
  ...
  async upload(file: Express.Multer.File, uploadedBy: string, folder?: string) {
    const type = file.mimetype.startsWith('image/') ? 'image'
      : file.mimetype.startsWith('video/') ? 'video' : 'document';
    const media = this.repo.create({
      uploadedBy, filename: file.filename, originalName: file.originalname,
      mimeType: file.mimetype, type, sizeBytes: file.size,
      url: `/uploads/${file.filename}`, folder: folder ?? 'general',
    });
