...
import { signAccessToken, signRefreshToken, verifyRefreshToken, hashRefreshToken, getTokenExpiryDate } from '../../common/utils/token.util';
...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    const user = this.userRepo.create({ email: dto.email, passwordHash, fullName: dto.fullName, phone: dto.phone });
  ...
  async refresh(rawToken: string, ip: string, userAgent: string) {
    let payload: any;
    try { payload = verifyRefreshToken(rawToken); } catch { throw new UnauthorizedError('Invalid refresh token'); }
    ...
    if (new Date() > storedToken.expiresAt) throw new UnauthorizedError('Refresh token expired');
